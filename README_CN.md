⚠️ 项目说明（复现者补充）
本仓库基于 原作者 Yang Jianhua 和 Dai Kun 的 YOWOv2 项目进行复现，所有核心代码、模型结构、实验配置均源自原项目，仅补充复现相关说明，不修改原作者核心工作。
原项目地址：https://github.com/yjh0410/YOWOv2，感谢原作者的开源贡献，如需引用，请参考原作者论文（详见文末参考文献）。

复现者工作说明

本人主要工作为 项目复现与环境适配，具体如下：
- 将原项目代码完整复刻至本仓库，并对部分内容进行修改主要改进eval保证代码算出ucf24数据集的准确率；
- 验证并适配原项目的环境配置，补充环境安装过程中的注意事项，确保不同设备可顺利搭建运行环境；
- 测试原项目提供的训练、测试、验证命令，确保命令可正常执行，复现原作者给出的实验效果；

- 保留原项目所有文件结构、可视化结果、数据集链接及实验参数，不修改原作者核心逻辑和实验结论；

注：本仓库仅用于个人学习和项目复现，不涉及原项目的修改、二次开发，所有版权归原作者所有。
