...
def getBoundingBoxes(directory,
    ...
    # 【修改点 1】使用 recursive glob 查找所有子目录下的 txt 文件
    # 注意：glob.glob 的 recursive 功能需要 pattern 为 '**/*.txt'
    search_pattern = os.path.join(directory, "**", "*.txt")
    files = glob.glob(search_pattern, recursive=True)

    if not files:
        print(f"Error: No .txt files found in {directory} (checked recursively).")
        return allBoundingBoxes, allClasses

    files.sort()
    # print(files)
    print(f"Found {len(files)} ground truth/detection files in {directory}.")  # 调试信息

    # Read GT detections from txt file
    for f in files:
        # 【修改点 2】从完整路径中提取文件名 (不带扩展名) 作为 imageName
        # 原逻辑是直接取 f，现在 f 是包含路径的，需要提取纯文件名
        nameOfImage = os.path.splitext(os.path.basename(f))[0]
...
if __name__ == '__main__':
    groundtruths_ucf = r"E:\study\daima\ucf24\labels"
    detect=r"E:\study\daima\YOWOv2-master\results\ucf_detections\yowo_v2_nano\detections_1"
    # evaluate_frameAP(groundtruths_ucf, detect)
