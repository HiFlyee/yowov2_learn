...
def parse_args():
    ...
    parser.add_argument('-tbs', '--test_batch_size', default=1, type=int,
                        help='test batch size on a single GPU.')
    parser.add_argument('-accu', '--accumulate', default=4, type=int,
                        help='gradient accumulate.')
    parser.add_argument('-lr', '--base_lr', default=0.00005, type=float, #0.0001  0.0000125 0.00005   0.000025  0.0000125
                        help='base lr.')
    ...
    parser.add_argument('--max_epoch', default=12, type=int,
                        help='max epoch.')
    parser.add_argument('--lr_epoch', nargs='+', default=[1,2,3], type=int,
                        help='lr epoch to decay')

    # Model
    parser.add_argument('-v', '--version', default='yowo_v2_nano', type=str,
                        help='build YOWOv2')
    parser.add_argument('-r', '--resume', default=r"E:\study\daima\YOWOv2-master\weights\ucf24\yowo_v2_nano\yowo_v2_nano_epoch_7.pth", type=str,
                        help='keep training')
    ...
    parser.add_argument('-m', '--memory', action="store_true", default=True,
                        help="memory propagate.")

    # Dataset
    parser.add_argument('-d', '--dataset', default='ucf24',
                        help='ucf24, ava_v2.2')
    parser.add_argument('--root', default=r'E:\study\daima',
                        help='data root')
    ...
    parser.add_argument('-fl', '--focal_loss', action="store_true", default=True,
...
def train():
    ...
    # if args.resume:
    #     print(f'[Resume] 从 epoch {start_epoch} 恢复训练，强制推进学习率调度器...')
    #     # 直接让调度器前进 start_epoch 步
