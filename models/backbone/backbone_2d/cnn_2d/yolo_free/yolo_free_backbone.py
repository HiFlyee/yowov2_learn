...
class ELANBlock(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, expand_ratio=0.5, model_size='large', act_type='silu', depthwise=False):
        ...
        inter_dim = int(in_dim * expand_ratio)  #中间层通道数
        if model_size == 'tiny':
            depth = 1
            ...
            depth = 3
        self.cv1 = Conv(in_dim, inter_dim, k=1, act_type=act_type)# 组件1: 1×1卷积分支1
        self.cv2 = Conv(in_dim, inter_dim, k=1, act_type=act_type)# 组件2: 1×1卷积分支2
        self.cv3 = nn.Sequential(*[
            Conv(inter_dim, inter_dim, k=3, p=1, act_type=act_type, depthwise=depthwise)
            for _ in range(depth)
        ])# 组件3: 3×3卷积序列1 (深度=depth)
...
class DownSample(nn.Module):
    def __init__(self, in_dim, act_type='silu', norm_type='BN'):
        super().__init__()
        inter_dim = in_dim // 2
        self.mp = nn.MaxPool2d((2, 2), 2)                                #output：[B, C, H/2, W/2]
        self.cv1 = Conv(in_dim, inter_dim, k=1, act_type=act_type, norm_type=norm_type)  #output：[B, C/2, H/2, W/2]
    ...
    def forward(self, x):
        ...
        x1 = self.cv1(self.mp(x))  #最大池化+逐点卷积
        x2 = self.cv2(x)           #逐点卷积+卷积
...
def build_elannet(model_name='elannet_large'):
    ...
    if model_name == 'elannet_large':
        backbone = ELANNet_Large()      #骨干网络
        feat_dims = [512, 1024, 1024]   #特征维度信息
...
class ShuffleV2Block(nn.Module):
    def __init__(self, inp, oup, stride):
        ...
        if self.stride > 1:
            ...
            self.branch1 = nn.Sequential()
        #深度卷积3×3(stride) → BN → 1×1卷积 → BN → ReLU
        #功能: 下采样 + 通道压缩
        ...
        #1×1卷积 → BN → ReLU → 深度卷积3×3 → BN → 1×1卷积 → BN → ReLU
    @staticmethod
...
class ShuffleNetV2(nn.Module):
    def __init__(self,
                 model_size='1.0x',
                 out_stages=(2, 3, 4),   #输出哪些阶段的特征图
                 with_last_conv=False,
                 kernal_size=3):
        super(ShuffleNetV2, self).__init__()
        print('model size is ', model_size)

        self.stage_repeats = [4, 8, 4]    #阶段重复次数
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(input_channels, output_channels, 3, 2, 1, bias=False),#下采样2倍
